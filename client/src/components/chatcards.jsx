  ...
  return (
    ...
    >
      <span className="h-[45px] w-[45px] bg-char text-smoke font-black rounded-full flex justify-center items-center">
        {photoURL == null ? username?.charAt(0).toUpperCase() :
          <img src={photoURL} alt="missing" className='bg-cover h-full w-full rounded-full' />
        }
      </span>
      ...
      <div className="flex flex-col justify-start">
        <span className={`${uid == active ? "text-light font-cursive tracking-wider font-extrabold text-[15px]" : "text-dark font-extrabold text-[14px]"}`}>
          <span className={`${userName?.uid == uid && 'font-cursive font-black'}`}>{userName?.uid == uid ? 'For Me From Past' : username}</span>
        </span>
